...
/* Putting single pixel values. */
inline void putPixval1(void *p, dword val)
{
	*(uint8 *) p = val.bytes[0];
}

inline void putPixval2(void *p, dword val)
{
	*(Uint16 *) p = *(Uint16 *) &val.bytes;
}

inline void putPixval3(void *p, dword val)
{
	((uint8 *) p)[0] = val.bytes[0];
	((uint8 *) p)[1] = val.bytes[1];
	((uint8 *) p)[2] = val.bytes[2];
}

inline void putPixval4(void *p, dword val)
{
	*(Uint32 *) p = val;
}


inline dword getPixval3(void *p)
{
	dword ret;

	ret.bytes[0] = ((uint8 *)p)[0];
	ret.bytes[1] = ((uint8 *)p)[1];
	ret.bytes[2] = ((uint8 *)p)[2];
...
*/
int fastPixelColorNolock(SDL_Surface * dst, Sint16 x, Sint16 y, dword color)
{
	int bpp;
	Uint8 *p;
...
\returns Returns 0 on success, -1 on failure.
*/
int fastPixelColorNolockNoclip(SDL_Surface * dst, Sint16 x, Sint16 y, dword color)
{
	int bpp;
	Uint8 *p;
...
\returns Returns 0 on success, -1 on failure.
*/
int fastPixelColor(SDL_Surface * dst, Sint16 x, Sint16 y, dword pixval)
{
	int result;
	...
	*/
	...
	result = fastPixelColorNolock(dst, x, y, pixval);
...
int fastPixelRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	* Setup color 
	*/
	color = PixelFmt::encode(r, g, b, a, dst->nformat);
...
\returns Returns 0 on success, -1 on failure.
*/
int _putPixelAlpha(SDL_Surface *dst, Sint16 x, Sint16 y, dword color, Uint8 alpha)
{
	SDL_PixelFormat *format;
	OSn:GFX::PixelFmt *nformat;
		...
		y >= clip_ymin(dst) && y <= clip_ymax(dst))
	{
			...
			{		/* Assuming 8-bpp */
				...
				if (alpha == 255) {
					*pixel = color.bytes[0];
					...
					OSn::Color32 dColor = nformat->palette.colors[*pixel];
					OSn::Color32 sColor = nformat->palette.colors[color.bytes[0]];
					...
					*pixel = PixelFmt::encode(RGBA(R, G, B), nformat).bytes[0];
				}
			}
			break;
		...
		case 2:
			{		/* Probably 15-bpp or 16-bpp */
				...
				if (alpha == 255) {
					putPixval2(pixel, color);
				} else {
					Uint16 R, G, B, A = 0xff;
					Uint16 dc = *pixel;

					PixelFmt::decode(color, nformat, (uint8&) sR, (uint8&) sG, (uint8&) sB, (uint8&) sA);

					PixelFmt::decode((dword) dc, nformat, (uint8&) dR, (uint8&) dG, (uint8&) dB, (uint8&) dA);

					R = (dR + ((sR - dR) * alpha >> 8));
					G = (dG + ((sG - dG) * alpha >> 8));
					B = (dB + ((sB - dB) * alpha >> 8));

					if (nformat->rgba.a_size != 0) {
						A = (dA + ((sA - dA) * alpha >> 8));
					}

					dword encoded = PixelFmt::encode(RGBA(R, G, B, A), nformat);
					putPixval2((void *) pixel, encoded);
				}
			}
			break;
		...
		case 3:
			{		/* Slow 24-bpp mode, usually not used */
				...
				PixelFmt::decode(color,  nformat, (uint8&) sR, (uint8&) sG, (uint8&) sB, (uint8&) sA);
				PixelFmt::decode(getPixval3(pixel), nformat, (uint8&) dR, (uint8&) dG, (uint8&) dB, (uint8&) dA);

				if (alpha == 255) {
					putPixval3(pixel, color);
				} else {
					R = dR + ((sR - dR) * alpha >> 8);
					G = dG + ((sG - dG) * alpha >> 8);
					B = dB + ((sB - dB) * alpha >> 8);

					dword encoded = PixelFmt::encode((uint8&) R, (uint8&) G, (uint8&) B, (uint8&) sA, nformat);
					putPixval3(pixel, encoded);
				}
			}
			break;
		...
		case 4:
			{		/* Probably :-) 32-bpp */
				...
				if (alpha == 255) {
					putPixval4(pixel, color);
				} else {
					Uint32 dc = *pixel;

					PixelFmt::decode(color,  nformat, (uint8&) sR, (uint8&) sG, (uint8&) sB, (uint8&) sA);
					PixelFmt::decode(*pixel, nformat, (uint8&) dR, (uint8&) dG, (uint8&) dB, (uint8&) dA);
					...
					if (nformat->rgba.a_size != 0) {
						...
						A = (dA | GFX_ALPHA_ADJUST_ARRAY[alpha & 255]); // make destination less transparent...
#else
						A = (dA + ((sA - dA) * alpha >> 8));
#endif
					}

					putPixval4(pixel, PixelFmt::encode(RGBA(R, G, B, A), nformat));
				}
			}
			break;
#endif
...
#error "Not implemented for OSn::GFX::PixelFormat yet."
...
\returns Returns 0 on success, -1 on failure.
*/
int pixelColor(SDL_Surface * dst, Sint16 x, Sint16 y, OSn::Color32 color)
{
	Uint8 alpha;
	dword en_color;
	int result = 0;
	...
	*/
	alpha = color.alpha;
	en_color = PixelFmt::encode(color, dst->nformat);

	/*
	* Draw 
	*/
	result = _putPixelAlpha(dst, x, y, en_color, alpha);
...
\param dst The surface to draw on.
\param x X (horizontal) coordin	ate of the pixel.
\param y Y (vertical) coordinate of the pixel.
\param color The color value of the pixel to draw.

\returns Returns 0 on success, -1 on failure.
*/
int pixelColorNolock(SDL_Surface * dst, Sint16 x, Sint16 y, OSn::Color32 color)
{
	Uint8 alpha;
	dword en_color;
	int result = 0;
...
\returns Returns 0 on success, -1 on failure.
*/
int _filledRectAlpha(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, dword color, Uint8 alpha)
{
	PixelFmt *nformat;
	Uint32 sR, sG, sB, sA;
	Uint32 dR, dG, dB, dA;
				...
				for (x = x1; x <= x2; x++) {
					if (alpha == 255) {
						*(row + x) = color.bytes[0];
						...
						dColor = nformat->palette.colors[*pixel];
						dR = dColor.red;
						dG = dColor.green;
						dB = dColor.blue;
						...
						*pixel = PixelFmt::encode(RGBA(R, G, B), nformat);
					}
				}
			}
		}
...
\returns Returns 0 on success, -1 on failure.
*/
int filledRectAlpha(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, OSn::Color32 color)
{
	Uint8 alpha;
	Uint32 en_color;
	int result = 0;
	...
	* Draw
	*/
	result = _filledRectAlpha(dst, x1, y1, x2, y2, en_color, alpha);
...
\returns Returns 0 on success, -1 on failure.
*/
int _HLineAlpha(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, OSn::Color32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
int _VLineAlpha(SDL_Surface * dst, Sint16 x, Sint16 y1, Sint16 y2, OSn::Color32 color)
{
...
\returns Returns 0 on success, -1 on failure.
*/
int pixelColorWeight(SDL_Surface * dst, Sint16 x, Sint16 y, OSn::Color32 color, Uint32 weight)
{
	color.alpha = (color.alpha * weight) >> 8;
...
\returns Returns 0 on success, -1 on failure.
*/
int pixelColorWeightNolock(SDL_Surface * dst, Sint16 x, Sint16 y, OSn::Color32 color, Uint32 weight)
{
	color.alpha = (color.alpha * weight) >> 8;

	return (pixelColorNolock(dst, x, y, color));
}
...
int pixelRGBA(SDL_Surface * dst, Sint16 x, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	*/
	if (a == 255) {
		...
		* Setup color 
		*/
		dword color = PixelFmt::encode(RGBA(r, g, b, a), dst->nformat);
		/*
		* Draw 
		*/
		return (fastPixelColor(dst, x, y, color));
		...
		* Draw 
		*/
		return pixelColor(dst, x, y, RGBA(r, g, b, a));
	}
}
...
\returns Returns 0 on success, -1 on failure.
*/
int hlineColor(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, OSn::Color32 color)
{
...
int hlineRGBA(SDL_Surface * dst, Sint16 x1, Sint16 x2, Sint16 y, Uint8 r, Uint8 g, Uint8 b, Uint8 a)
{
	...
	* Draw
	*/
	return (hlineColor(dst, x1, x2, y, RGBA(r, g, b, a)));
}
...
\returns Returns 0 on success, -1 on failure.
*/
int vlineColor(SDL_Surface * dst, Sint16 x, Sint16 y1, Sint16 y2, OSn::Color32 color)
{
	Sint16 left, right, top, bottom;
	Uint8 *pixel, *pixellast;
	int dy;
...
\returns Returns 0 on success, -1 on failure.
*/
int boxColor(SDL_Surface * dst, Sint16 x1, Sint16 y1, Sint16 x2, Sint16 y2, OSn::Color32 color)
{
	Sint16 left, right, top, bottom;
	Uint8 *pixel, *pixellast;
	int x, dx;
...
\param color The color value of the arc to draw.
			...
			do {
				...
				if (((ok != k) && (oj != k)) || ((oj != j) && (ok != j)) || (k != j)) {
					...
					if (k > 0) {
						ypk = y + k;
						ymk = y - k;
						result |= fastPixelColorNolock(dst, xmh, ypk, pixval);
						result |= fastPixelColorNolock(dst, xph, ypk, pixval);
						result |= fastPixelColorNolock(dst, xmh, ymk, pixval);
						result |= fastPixelColorNolock(dst, xph, ymk, pixval);
					} else {
						result |= fastPixelColorNolock(dst, xmh, y, pixval);
						result |= fastPixelColorNolock(dst, xph, y, pixval);
					}
					ok = k;
					xpi = x + i;
					xmi = x - i;
					if (j > 0) {
						ypj = y + j;
						ymj = y - j;
						result |= fastPixelColorNolock(dst, xmi, ypj, pixval);
						result |= fastPixelColorNolock(dst, xpi, ypj, pixval);
						result |= fastPixelColorNolock(dst, xmi, ymj, pixval);
						result |= fastPixelColorNolock(dst, xpi, ymj, pixval);
					} else {
						result |= fastPixelColorNolock(dst, xmi, y, pixval);
						result |= fastPixelColorNolock(dst, xpi, y, pixval);
					}
					oj = j;
				}
			...
			do {
				...
				if (((oi != i) && (oh != i)) || ((oh != h) && (oi != h) && (i != h))) {
					...
					if (i > 0) {
						ypi = y + i;
						ymi = y - i;
						result |= fastPixelColorNolock(dst, xmj, ypi, pixval);
						result |= fastPixelColorNolock(dst, xpj, ypi, pixval);
						result |= fastPixelColorNolock(dst, xmj, ymi, pixval);
						result |= fastPixelColorNolock(dst, xpj, ymi, pixval);
					} else {
						result |= fastPixelColorNolock(dst, xmj, y, pixval);
						result |= fastPixelColorNolock(dst, xpj, y, pixval);
					}
					oi = i;
					xmk = x - k;
					xpk = x + k;
					if (h > 0) {
						yph = y + h;
						ymh = y - h;
						result |= fastPixelColorNolock(dst, xmk, yph, pixval);
						result |= fastPixelColorNolock(dst, xpk, yph, pixval);
						result |= fastPixelColorNolock(dst, xmk, ymh, pixval);
						result |= fastPixelColorNolock(dst, xpk, ymh, pixval);
					} else {
						result |= fastPixelColorNolock(dst, xmk, y, pixval);
						result |= fastPixelColorNolock(dst, xpk, y, pixval);
					}
					oh = h;
				}
...
\param color The color value of the pie to draw.
...
\param color The color value of the filled pie to draw.
...
\param color The color value of the polygon to draw.
...
\param n Number of points in the vertex array. Minimum number is 3.
\param color The color value of the filled polygon to draw.
...
\param color The color value of the character to draw.
...
\param color The color value of the string to draw.
...
\param color The color value of the bezier curve to draw.
